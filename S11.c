...
int verificareEchilibru(NodArbore* root) { //stanga - dreapta
...
void adaugaMasinaInArboreEchilibrat(NodArbore** root, Masina masinaNoua) {
	if (*root) {
		...
		else adaugaMasinaInArboreEchilibrat(&(*root)->left, masinaNoua);
		//verificare echilibru
		int factorEchilibru = verificareEchilibru(*root);
		if (factorEchilibru == -2) { //dezechilibru dreapta
